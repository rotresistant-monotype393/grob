...
Twelve core modules are auto-available: `fs`, `strings`, `json`, `csv`, `env`,
`process`, `date`, `math`, `log`, `regex`, `path`, `format`. No import, no
ceremony.
...
Negative: twelve namespaces are always in scope. Potential for name collisions
